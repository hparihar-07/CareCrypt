...
const HealthShieldSVG = ({ className = "w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg opacity-90 drop-shadow-lg" }) => (
  <svg
    className={className}
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen px-6 py-16 text-white bg-gradient-to-r from-gray-950 to-gray-800">
      <div className="text-center max-w-4xl animate-fadeInUp">
        <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold leading-tight mb-7 tracking-tight">
        ...
        <div className="relative inline-block py-5 px-10 mb-12 rounded-xl border border-indigo-500 shadow-2xl shadow-indigo-500/50 transform rotate-3 transition-transform duration-300 hover:rotate-0">
          <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold leading-tight tracking-tight whitespace-nowrap text-green-400 select-none">
            CareCrypt
          </h2>
        </div>

        <p className="text-lg sm:text-xl md:text-2xl mb-14 text-gray-300 font-light max-w-2xl mx-auto leading-relaxed">
        ...
        <div className="flex flex-col sm:flex-row gap-6 justify-center">
          <Link
            to="/register"
            className="px-8 py-3 text-sm sm:px-10 sm:py-3 bg-green-400 text-white font-semibold rounded-lg shadow-lg hover:bg-sky-600 focus:outline-none focus:ring-4 focus:ring-sky-400 transition-transform transform hover:scale-105 focus:scale-105"
          ...
          <Link
            to="/about"
            className="px-8 py-3 text-sm sm:px-10 sm:py-3 border-2 border-sky-500 text-sky-500 font-semibold rounded-lg hover:bg-sky-500 hover:text-white focus:outline-none focus:ring-4 focus:ring-sky-400 transition-transform transform hover:scale-105 focus:scale-105"
      ...
      <section className="mt-16 text-center max-w-4xl">
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold mb-10 text-white leading-tight tracking-tight">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
          {[ 
            ...
            <article
              ...
              aria-label={title}
            >
              <h3 className="text-xl sm:text-2xl md:text-3xl font-semibold mb-4 text-teal-300">{title}</h3>
